import { showCommentsPopUp, hideCommentsPopUp, updateCommentList} from './commentsPopupViewer.js';
...
export default function addEventListnersToTheCommentsButtons() {
  ...
  comments.forEach((el) => {
    el.addEventListener('click', async (event) => {
      ...
      document.querySelector('#comment-form').addEventListener('submit', async (formEvent) => {
        formEvent.target.elements['comment-button'].disabled = true;
        formEvent.preventDefault();
        await postComment(getShowID(event.target.id), formEvent.target.elements['name'].value, formEvent.target.elements['comment'].value);
        await updateCommentList(getShowID(event.target.id));
